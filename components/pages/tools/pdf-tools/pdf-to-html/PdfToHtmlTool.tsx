  ...
  const handleConversion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/convert_pdf_to_html",
        {
          method: "POST",
          body: formData,
        }
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400 text-sm">
              Upload a PDF file to convert it to HTML format
            </p>
          </div>
          <FileUpload
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-gray-300 text-lg font-medium mb-2">
                Convert {uploadedFile.name} to HTML File
              </h3>
              <p className="text-gray-400 text-sm mb-4">
                This will convert your PDF to HTML format (.html)
              </p>
            </div>
            ...
            >
              {isProcessing ? "Converting..." : "Convert to HTML File"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="mt-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-white/10">
            ...
            <button
              onClick={() =>
                openMonetizationModal(
                  `${uploadedFile?.name.replace(".pdf", "")}_converted.html`,
                  "HTML",
