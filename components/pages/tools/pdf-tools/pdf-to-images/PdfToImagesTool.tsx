  ...
  const handleConversion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/convert_pdf_to_images",
        {
          method: "POST",
          body: formData,
        }
      ...
      setResult({
        type: "error",
        message: "Error converting PDF to images. Please try again.",
      });
    } finally {
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400 text-sm">
              Upload a PDF file to convert it to image files
            </p>
          </div>
          <FileUpload
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-gray-300 text-lg font-medium mb-2">
                Convert {uploadedFile.name} to Image Files
              </h3>
              <p className="text-gray-400 text-sm mb-4">
                This will convert your PDF pages to individual image files
              </p>
            </div>
            ...
            >
              {isProcessing ? "Converting..." : "Convert to Image Files"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="mt-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-white/10">
            ...
            <button
              onClick={() =>
                openMonetizationModal(
                  `${uploadedFile?.name.replace(".pdf", "")}_images.zip`,
                  "ZIP",
                  result.data.download_url || result.data.images_zip_url
                )
              }
          ...
          <div className="p-6">
            <div className="text-center">
              <p className="text-gray-400 text-sm mb-4">
                Your PDF has been converted to {result.data.total_images || 0}{" "}
                image files
              </p>
            </div>
          </div>
