...
export const HtmlToPdfTool: React.FC<HtmlToPdfToolProps> = ({
  ...
  const handleProcessDocument = useCallback(async () => {
    ...
    try {
      ...
      // Save HTML file first, then convert using template
      const formData = new FormData();
      formData.append("html", uploadedFile);
      ...
      // Get HTML info including page count
      const htmlInfoResponse = await fetch(`/api/html_info/${filename}`);
      if (htmlInfoResponse.ok) {
        const htmlInfo = await htmlInfoResponse.json();
        console.log("📄 HTML info:", htmlInfo);
        setTotalPages(htmlInfo.page_count || 1);
      } else {
        console.warn("Failed to get HTML info, defaulting to 1 page");
        setTotalPages(1);
      }
      ...
      alertModal.showError("Error", "Failed to process HTML file");
    } finally {
  ...
  const handleToolSelect = (toolId: string) => {
    ...
    if (iframe && iframe.contentWindow) {
      ...
      // If convert tool is selected, send additional message to configure conversion behavior
      if (toolId === "convert") {
        iframe.contentWindow.postMessage(
          {
            type: "CONFIGURE_CONVERT_MODAL",
            showOnce: true,
            targetFormat: "pdf",
          },
          "*"
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      } else if (event.data.type === "PDF_CONVERTED") {
        console.log("🔄 PDF converted:", event.data);
  ...
  const generatePageThumbnails = () => {
    ...
    return Array.from({ length: totalPages }, (_, index) => ({
      ...
      thumbnailUrl: `/api/html_thumbnail/${uploadedFile.name}/${index + 1}`,
          ...
          <div className="p-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400">
                Upload an HTML file to convert to PDF format
              </p>
            </div>
