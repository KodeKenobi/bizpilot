...
export const ImageToPdfTool: React.FC<ImageToPdfToolProps> = ({
  ...
  const handleProcessDocument = useCallback(async () => {
    ...
    try {
      ...
      // Save image file first, then convert using template
      ...
      // Get image info including page count
      const imageInfoResponse = await fetch(`/api/image_info/${filename}`);
      if (imageInfoResponse.ok) {
        const imageInfo = await imageInfoResponse.json();
        console.log("📄 Image info:", imageInfo);
        setTotalPages(imageInfo.page_count || 1);
      } else {
        console.warn("Failed to get image info, defaulting to 1 page");
        setTotalPages(1);
      }
      ...
      alertModal.showError("Error", "Failed to process image file");
    } finally {
  ...
  const generatePageThumbnails = () => {
    ...
    return Array.from({ length: totalPages }, (_, index) => ({
      ...
      thumbnailUrl: `/api/image_thumbnail/${uploadedFile.name}/${index + 1}`,
          ...
          <div className="p-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400">
                Upload an image file to convert to PDF format
              </p>
            </div>
