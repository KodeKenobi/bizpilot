...
export const CompressTool: React.FC<CompressToolProps> = ({
  ...
  const handleToolSelect = (toolId: string) => {
    ...
    if (iframe && iframe.contentWindow) {
      ...
      // If compress tool is selected, send additional message to configure compression behavior
      if (toolId === "compress") {
        iframe.contentWindow.postMessage(
          {
            type: "CONFIGURE_COMPRESS_MODAL",
            showOnce: true,
          },
          "*"
        );
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      } else if (event.data.type === "PDF_COMPRESSED") {
        console.log("🗜️ PDF compressed:", event.data);
