...
export const WordToPdfTool: React.FC<WordToPdfToolProps> = ({
  ...
  const handleProcessDocument = useCallback(async () => {
    ...
    try {
      ...
      // Save Word file first, then convert using template
      const formData = new FormData();
      formData.append("word", uploadedFile);
      ...
      const docInfoResponse = await fetch(`/api/doc_info/${filename}`);
      if (docInfoResponse.ok) {
        const docInfo = await docInfoResponse.json();
        console.log("📄 Document info:", docInfo);
        setTotalPages(docInfo.page_count || 1);
      } else {
        console.warn("Failed to get document info, defaulting to 1 page");
        setTotalPages(1);
      }
  ...
  const generatePageThumbnails = () => {
    ...
    return Array.from({ length: totalPages }, (_, index) => ({
      ...
      thumbnailUrl: `/api/doc_thumbnail/${uploadedFile.name}/${index + 1}`,
          ...
          <div className="p-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400">
                Upload a Word document to convert to PDF format
              </p>
            </div>
