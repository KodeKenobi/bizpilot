...
export default function CreateCampaignPage() {
  ...
  useEffect(() => {
    ...
    if (stateStr) {
      ...
      if (state.step === 3 && !state.completed) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update onboarding state to step 4
      const stateStr = localStorage.getItem("onboarding_state");
      if (stateStr) {
        const state = JSON.parse(stateStr);
        if (state.step === 3) {
             localStorage.setItem("onboarding_state", JSON.stringify({ step: 4, completed: false }));
        }
      }

      router.push(`/campaigns/${data.campaign.public_id}?new=1`);
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-black pt-24 pb-12 px-8">
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div id="email-options-section" className="border border-gray-800 rounded-xl p-5 space-y-4">
        ...
        <OnboardingWalkthrough
          steps={[
            {
              title: "Campaign Name",
              description: "Name your campaign to track it later.",
              targetId: "campaignName",
            },
            {
              title: "Your Identity",
              description: "IMPORTANT: Fill this completely. The AI uses this exact info to fill out contact forms on your behalf.",
              targetId: "sender-info-section",
            },
            {
              title: "Email Options",
              description: "Decide if you want to email contacts when no form is found, and add a CC address if needed.",
              targetId: "email-options-section",
            },
            {
              title: "Your Message",
              description: "Type the message you want the AI to send.",
              targetId: "message",
            },
            {
              title: "Launch Campaign",
              description: "Launch your campaign workspace.",
              targetId: "create-campaign-submit",
            },
          ]}
          run={showWalkthrough}
