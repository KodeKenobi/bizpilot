      ...
      <div className="min-h-[80vh] flex items-center justify-center mb-32">
        <div className="text-center max-w-4xl">
          ...
          <Link href="/campaigns/upload">
            <button id="create-campaign-btn" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
...
export default function CampaignsPage() {
  ...
  const handleDeleteClick = (campaignId: number) => {
    // Explicitly prevent any default browser behavior or bubbling
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // Only admins should use the admin endpoint, regular users use the standard one
      const isAdmin = user?.role === 'admin' || user?.role === 'super_admin';
      const endpoint = isAdmin 
        ? `/api/campaigns/admin/${campaignToDelete}/delete` 
        : `/api/campaigns/${campaignToDelete}`;

      const response = await fetch(endpoint, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          // Ensure we send auth token for admin requests 
  ...
  const onboardingSteps: any[] = [
    {
      ...
      description: "Keep an eye on your daily usage limits and processing statistics right here.",
      targetId: "usage-stats",
    },
    {
      title: "Track Your Results",
      description: "Once started, your campaigns will appear here. You can click on them for details.",
      targetId: "campaigns-list",
    },
    {
      title: "Start Your First Campaign",
      description: "Ready to go? Click 'Start Creating' to upload your company list.",
      targetId: "create-campaign-btn",
    },
  ];
  ...
  useEffect(() => {
    ...
    // Legacy check
    const legacySeen = localStorage.getItem("hasSeenCampaignsWalkthrough");
    
    if (!state && !legacySeen) {
      // First time ever
      setShowWalkthrough(true);
    } else if (state && state.step === 1 && !state.completed) {
      // Resuming step 1
      setShowWalkthrough(true);
    }
  }, []);
  ...
  const handleWalkthroughComplete = () => {
    ...
    localStorage.setItem("onboarding_state", JSON.stringify({ step: 2, completed: false }));
    setShowWalkthrough(false);
    router.push("/campaigns/upload");
  };

  const handleWalkthroughSkip = () => {
    // Mark as fully completed/skipped
    localStorage.setItem("onboarding_state", JSON.stringify({ step: 1, completed: true, skipped: true }));
    setShowWalkthrough(false);
  };
    ...
    <div className="min-h-screen bg-background pt-20 pb-12 px-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              {dailyUsage && (
                <div id="usage-stats" className="flex items-center gap-3 min-w-0 px-3 py-1 bg-white/5 rounded-lg border border-white/10">
        ...
        {campaigns.length === 0 ? (
          ...
          <div id="campaigns-list" className="space-y-6">
            {/* Campaigns Grid */}
