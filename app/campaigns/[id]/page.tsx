...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    if (searchParams?.get("new") === "1" && campaign?.status === "pending") {
      ...
      if (stateStr) {
        ...
        if (state.step === 4 && !state.completed) {
          setShowWalkthrough(true);
        }
      }
    }
  }, [searchParams, campaign?.status]);
  ...
  return (
    <div className="min-h-screen bg-black pt-24 pb-12 px-8">
      <div className="max-w-[1400px] mx-auto">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {companies.filter((c) => c.status === "pending").length > 0 && (
                <button
                  ...
                  className={`group flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold transition-all rounded-lg disabled:opacity-50 disabled:cursor-not-allowed ${
                    shouldPulsate ? "animate-pulsate shadow-[0_0_15px_rgba(59,130,246,0.5)] scale-105" : ""
                  }`}
                  onClick={() => {
        ...
        <div id="campaign-stats" className="grid grid-cols-6 gap-4 mb-8">
          {[
            {
              label: "Total",
              value: stats.total,
        ...
        steps={[
          {
            ...
            description: "Track processed, successful, and failed leads here in real-time.",
            targetId: "campaign-stats",
          },
          {
            title: "Start Processing",
            description: "Click 'Rapid All' to start the high-speed processing engine!",
            targetId: "rapid-fire-btn",
          },
        ]}
        run={showWalkthrough}
        onComplete={() => {
          setShowWalkthrough(false);
          localStorage.setItem("onboarding_state", JSON.stringify({ step: 4, completed: true }));
        }}
        onSkip={() => {
          setShowWalkthrough(false);
          localStorage.setItem("onboarding_state", JSON.stringify({ step: 4, completed: true, skipped: true }));
        }}
      />
