...
export default function TestingPage() {
  ...
  const [selectedTool, setSelectedTool] = useState<string | null>("image");
...
This is an automated notification from the Trevnoctilla Admin Dashboard
  ...
  const testImageConverter = async (visualMode: boolean = false) => {
    ...
    try {
      ...
      // Quick Test: Automated Monetization Click
      if (!visualMode) {
        setImageTestStep('Running background ad interaction...');
        try {
          if (hiddenIframeRef.current) {
            hiddenIframeRef.current.src = "/tools/image-converter";
            const automationResults = await automateImageConverter(hiddenIframeRef.current, {
              setStep: setImageTestStep,
              setProgress: setImageTestProgress,
              onUpdate: (res) => {
                setTestResults(prev => ({ 
                  ...prev, 
                  imageConverter: { 
                    ...results, 
                    tests: [...results.tests, ...res.tests] 
                  } 
                }));
              }
            });
            results.tests.push(...automationResults.tests);
          }
        } catch (e) {
          console.error("Quick Test Automation Error:", e);
        } finally {
          if (hiddenIframeRef.current) {
            try {
              hiddenIframeRef.current.src = "about:blank";
            } catch (e) {}
          }
        }
      }
