...
export async function POST(request: NextRequest) {
  try {
    ...
    // Always use canonical ITN route for PayFast notifications.
    // Required for lifetime extension unlock flow.
    paymentData.notify_url = "https://www.trevnoctilla.com/api/payment/notify";
    ...
    // Force deterministic marker for extension lifetime checkout.
    // This is used by ITN processing to detect LeakFinder lifetime purchases.
    if (is_extension_lifetime === true) {
      paymentData.custom_str1 = "leakfinder_pro_lifetime";
    } else if (custom_str1) {
      paymentData.custom_str1 = String(custom_str1).trim();
    }
